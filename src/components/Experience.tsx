        ...
        <div className='flex flex-col items-center min-h-screen scroll-mt-40 items-center' id='experience'>
            ...
            {t.experience.data.map((item, index) => (
                <motion.div key={index} variants={containerVariants} viewport={{ once: true, amount: 0.3 }} initial="hidden"
                    ...
                    <motion.h3 className='font-title font-semibold text-2xl mb-2 lg:text-3xl' variants={textVariants}>{item.company}</motion.h3>
                    <motion.p className='font-title font-semibold text-xl text-[var(--primary)] mb-4 lg:text-2xl' variants={textVariants}>{item.date}</motion.p>
                    <Line />
